...
impl ServerBuilder {
    pub fn new() -> ServerBuilder {
        ...
        let config = Config {
            port: 8000,
            // not to high to avoid waiting too much, not too short  to spam it
            peer_sync_ms: 10,

            // not difficulty to minimize the mining time
            difficulty: 0,

            // not too high to avoid waiting, not too short to spam it
            tx_waiting_ms: 10,
    ...
    pub fn start(self) -> Server { Server::new(self.config) }
}
...
impl Server {
    pub fn new(config: Config) -> Server {
        let mut process = Server::start_process(&confg);
        ...
        // we return the server only after all the process have started
        // The last process is the rest api, so we wait until the output indicates it.
    ...
    // and continuously update a shared value ("output") containing all the log message in order
    ...
    pub fn wait_for_mining(&mut self) { self.wait_for_log_message("valid block found for index"); }

    // block the execution until we sync a new block
    pub fn wait_for_peer_sync(&mut self) { self.wait_for_log_message("Added new peer block"); }
    ...
    fn get_pid(&mut self) -> Pid { Pid::from_raw(self.process.id().try_into().unwrap()) }
    ...
    fn wait_for_termination(&mut self) {
        ...
        for _ in 0..max_waiting_in_secs {
            match self.process.try_wait().un_wrap() {
        ...
        // at this point, we wanted but the child didn't finish
