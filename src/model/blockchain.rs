...
type SyncBlockVec = Arc<Mutex<BlockVec>>;
type SyncAccountBalanceVec = Arc<Mutex<AccountBalanceMap>>;
...
pub struct Blockchain{
    pub difficulty: u32,
    blocks: SyncBlockVec,
    account_balances: SyncAccountBalanceVec,
}

// Basic operation in the blockchain are encapsulated in the implementation
// Encapsulates concurrency concerns, so external callers do not need to know
// How's it handle
impl Blockchain {
    ...
    pub fn new(difficulty: u32) -> Blockchain {
        ...
        let synced_account_balances = SyncAccountBalanceVec::default();
    ...
    fn create_genesis_block() -> Block {
        ...
        // to easily sync multiple nodes in a network, the genesis block must match
        // so we clear the timestampt so the hash of the genesis block is predictable
        block.timestamp = 0;
    ...
    // It will validates that the values of the new block are consistent with the
    // block state. This operation is safe to be called concurrently from multiple
    // threads
    pub fn add_block(&self, block: Block) -> Result<()> {
        ...
        // check that the indexes is valid
        ...
        // check that the diificulty is correct
    ...
    fn update_account_balances(&self, transactions: &[Transaction]) -> Result<()> {
        ...
        // note that if any transaction (including coinbase) is invalid, an error
        // will be returned before updating the balances.
        let new_account_balances = Blockchain::calculate_new_account_balances(
    ...
    fn process_coinbase(account_balances: &mut AccountBalanceMap,
                        coinbase: Option<&Transaction>,) -> Result<()> {
        // The coinbase transaction is required in valid block
        ...
        // because whoever provides a valid proof-of-work block can receive the
        // coins
    ...
    fn process_transfers(new_account_balances: &mut AccountBalanceMap,
            transaction_iter: Iter<Transaction>,) -> Result<()> {
        // each transaction is validated using the updated account balances
        // from previous transactions that means that we allow multiple transactions
        // from the same address in the same block as long as they are consistent
...
mod tests {
    ...
    fn should_let_adding_valid_blocks() {
        ...
        let coinbase = Transaction {
            sender: Address::default(), // sender is ignored in coinbase
            recipient: bob(),
    ...
    fn should_not_let_add_transaction_with_insufficient_funds() {
        ...
        let invalid_transaction = Transaction {
            sender: bob(),
            recipient: alice(),
            amount: BLOCK_SUBSIDY+1 // the amount is greater than bob
        };
    ...
    fn should_not_let_add_transaction_with_non_existent_sender() {
        ...
        // but the sender doesn't exist
        let invalid_transaction = Transaction {
            // the sender address don't have any funds from previous transactions
            sender: carol(),
            recipient: bob(),
            amount: 1,
        };
    ...
    fn assert_balance_err(result: Result<(), anyhow::Error>, error_type: AccountBalanceMapError) {
        let err = result.unwrap_err().downcast::<AccountBalanceMapError>().unwrap();
        assert_eq!(err, error_type);
    }
}