...
impl Runnable for Peer {
    fn run(&self) -> Result<()> { self.start() }
}

impl Peer {
    pub fn new(context: &Context) -> Peer {
        Peer {
            peer_address: context.config.peers.clone(),
    ...
    pub fn start(&self) -> Result<()> {
        ...
        info!(
            "Start peer system with peers: {}", self.peer_address.join(", ")
        );
    ...
    fn get_last_block_index(&self) -> usize { self.blockchain.get_last_block().index as usize }

    // retrieve new blocks from all peers and add them to the blockchain
    fn try_receive_new_blocks(&self) {
        for address in self.peer_address.iter() {
    ...
    // Retrieve only new blocks from a peer
    ...
    // Try to broadcast all new blocks to peers since last time we broadcast
    fn try_send_new_blocks(&self, last_send_block_index: usize) {
        ...
        for block in new_blocks.iter() {
            for address in self.peer_address.iter() {
                ...
                info!("Already send new block {} to peer {}", block.index, address);
            }
        }
    }

    // Return all new blocks added to the blockchain since th one with the indicated index
    fn get_new_blocks_since(&self, start_index: usize) -> Vec<Block> {
        ...
        self.blockchain.get_all_blocks().get(new_blocks_range).unwrap().to_vec()
    }
    ...
    fn send_block_to_peer(address: &str, block: &Block) {
        ...
        let request = Request::post(uri).header("Content-type", "application/json")
            .body(body).unwrap();
