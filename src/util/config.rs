...
// Encapsulates configuration to be used accross the application
...
impl Config {
    ...
    // Parses a singular value from an environment variable, accepting a default value if missing.
    ...
    // Parses a multiple value (Vec) from an environment variable, accepting a default value if missing.
...
mod tests {
    ...
    fn read_present_vec_envvar() {
        ...
        // let's remove the var at the end to not pollute the env
        env::remove_var(var_name);
    }
    ...
    fn read_invalid_envvar() {
        // envvars should have the "=" character in the name
