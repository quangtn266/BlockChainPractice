...
impl Miner {
    ...
    // Try to constantly calculate and append new valid blocks to the blockchains
    ...
    pub fn start(&self) -> Result<()> {
        ...
        loop {
            ...
            // do no try to mine a block if there are no transactions in the pool
            ...
            match mining_result {
                ...
                None => {
                    ...
                    return Err(MineError::BlockNotMined(index).into());
                }
            }
        }
    }
    ...
    // it will create blocks with difference "nonce" values until one has a hash that matches the difficulty
    // Returns either valid block (that satisfies the difficulty) or "None" if no block was found
...
mod tests {
    ...
    fn test_create_target_overflowing_difficulty() {
        ...
        // it must default to the max diffulty
    ...
    fn test_mine_block_found() {
        ...
        // this should be more than enough once to find a block with only 1 zero
        let max_nonce=1_000;
    ...
    fn test_mine_block_not_found() {
        // let's use high difficulty target to never find a block
    ...
    fn test_run_block_not_found() {
        // with a max_nonce so lwo and difficulty so high
    ...
    fn miner_address() -> Address { alice() }

    fn create_miner(difficulty: u32, max_nonce: u64) -> Miner {
        let miner_address = miner_address();
        let max_blocks: u64 = 1;
        let tx_waiting_ms: u64 = 1;
        let target = Miner::create_target(difficulty);

        let blockchain = Blockchain::new(difficulty);
        let pool = TransactionPool::new();

        Miner { miner_address, max_blocks, max_nonce, tx_waiting_ms, blockchain, pool, target,}
    }

    fn create_empty_block() -> Block { return Block::new(0, 0, BlockHash::default(), Vec::new()); }
